...
exports.doClick = function(element, logText){
    if (!element) throw new Error(`utils: Error --- doClick: element is not clickable`);

    return element.isEnabled()
        .then(() => element.click())
        .then(() => log.step('utils', 'doClick',`${logText}`))
        .catch((err) => {
            throw new Error(`utils: Error --- doClick: ${err.message}`);
        });
};
/**
...
exports.doSendKeys = function (element, text, logText) {
    if (!isString(text)) throw new Error(`utils: text is not an string`);

    return element.sendKeys(text)
        .then(() => log.step('utils', 'doSendKeys',`${logText}`))
      .catch((err) => {
          throw new Error(`utils: Error --- doSendKeys: ${err.message}`);
      });
};